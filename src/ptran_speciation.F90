...
!-------speciate aqueous solution
        ...
        if (myrank ==0) then
          if (ndloc1.eq.2) then
            if (mod(ireg,2).ne.0) then
              write(*,'(8x,"--> compute initial conditions: ", &
...
if (iact==6 .and. myrank==0) then
  write(iunit2,'( &
  &  " mole fraction H2O   = ",1pe14.4, &
  &  " (approx. ",1pe12.4,")"/ &
  &  " activity of water   = ",1pe14.4, &
  &  " osmotic coefficient = ",1pe14.4)') xh2o,xh2oapp, &
  &  ah2oloc,phih2o
      endif
...
!         write(*,*) 'treqres: ',iter,j,jph,joh,iph,ireg,ctot(j,ireg), &
!         eqhom(iph),ah2o0
...
!         (recommended)
          ...
          call henry   (xco2,x1m,tc,pco2,phico2,hpco2,poynco2)
          call cowat   (tc,pco2pa,dwat,uwat,ierr)
          call denmix  (tc,dwat,x1m,d1m)
          
          wmix = xmwc*xmww/(x1m*xmww+(1.d0-x1m)*xmwc)
          cco2 = x1m*d1m/xmwc*1.e-3

          akeq = wmix*hpco2*1.e-2/d1m/poynco2
          alogkeq = log10(akeq)
          eqgas(jco2g) = alogkeq-log10(phico2)
          
!         pstar = 10.d0**eqgas(jco2g)*cco2
          
!         write(*,*) 'treqrew: ',pco2,cco2,phico2,tc,alogkeq, &
!         eqgas(jco2g),pstar
...
!=========================================================================
      ...
      real*8 :: cx(*),gam(*),gamx(*),dgamdi(*)
      real*8 :: cc(*),alogac(ncmx),alogacx(ncxmx)
      ...
      if(ncmplx.gt.0) then
        ...
        do j = 1, ncomp
          ...
          if(z(j).ne.zero) then
            ...
            dgamdi(j)=-half*adebye*z(j)*z(j)/(sqrti* &
            afac*afac) + bextend(j)
          endif
        enddo
        didi=zero
