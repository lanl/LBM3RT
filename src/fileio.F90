...
! use paramtrsmod
  ...
  implicit none
  integer, parameter :: dbaselen = 256, strlen = 128
  save
...
! specify public variables
  public :: fiToUpper, fiToLower, fiWordToLower, fiWordToUpper,   &
            ...
            fiReadInt, fiReadLog, fiReadDouble, fiReadMultDouble, &
...
! ************************************************************************** !
!
! fiReadLog: reads and removes a logical value from a string
! author: PCL
! date: 10/23/04
!
! ************************************************************************** !
subroutine fiReadLog(string, int, ierr)
...
!  if (ierr /= 0) return
  ...
  do
    ...
    if (word(1:4) == 'SKIP') then
      do 
        read(fid,'(a128)',iostat=ierr) tempstring
        if (ierr /= 0) then
