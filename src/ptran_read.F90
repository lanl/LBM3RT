...
!           IONX, SORP, LBM
  ...
  real*8 :: fac,val,cec00,eqiex0,area00,cover0,eqsorp00,rkfsrf0,rkbsrf0, &
            sited00
...
!....................
    ...
    & "  pref0  = ",1pe12.4)') difaq,delhaq,difgas,dgexp, &
      ...
      tempini = temp0
      difaq = 1.d-4*difaq ! convert from cm^2/s -> m^2/s
...
!       enddo ! end loop over regions
      ...
      if (myrank == 0) then
        ...
        do isrc = 1, nblksrc
          ...
          write(iunit2,'("  #     time[s]     temp[C]     q[kg/s]")')
          do i = 1, nsrc
            write(iunit2,'(i3,1x,1p3e12.4)') i, &
            timesrc(i,isrc),tempsrc(i,isrc),qsrc(i,isrc)
          enddo
          write(iunit2,*) 'species            itype   ctot        ', &
          'constraint'
          do j = 1, ncomp
...
!....................
    ...
    case ('LBM')
      ...
      call fiReadStringErrorMsg('LBM',ierr)

      call fiReadInt(string,ntimes,ierr)
      call fiDefaultMsg('ntimes',ierr)
        
      call fiReadDouble(string,bsat,ierr)
      call fiDefaultMsg('bsat',ierr)
    
!     call fiReadInt(string,nbsat,ierr)
      call fiReadDouble(string,nbsat,ierr)
      call fiDefaultMsg('nbsat',ierr)
    
      call fiReadDouble(string,rhol,ierr)
      call fiDefaultMsg('rhol',ierr)
    
      call fiReadDouble(string,rhor,ierr)
      call fiDefaultMsg('rhor',ierr)
    
      call fiReadDouble(string,uin,ierr)
      call fiDefaultMsg('uin',ierr)
    
      call fiReadDouble(string,ff,ierr)
      call fiDefaultMsg('ff',ierr)
      
      call fiReadInt(string,ibndtp,ierr)
      call fiDefaultMsg('ibndtp',ierr)      
    
      call fiReadDouble(string,tau0,ierr)
      call fiDefaultMsg('tau0',ierr)
    
      call fiReadDouble(string,epsss,ierr)
      call fiDefaultMsg('epsss',ierr)
    
      call fiReadFlotranString(iunit1,string,ierr)
      call fiReadStringErrorMsg('LBM',ierr)

      call fiReadInt(string,icheck,ierr)
      call fiDefaultMsg('icheck',ierr)

      call fiReadInt(string,iwrite,ierr)
      call fiDefaultMsg('iwrite',ierr)

      call fiReadInt(string,ishift,ierr)
      call fiDefaultMsg('ishift',ierr)
      
      call fiReadInt(string,nrevx,ierr)
      call fiDefaultMsg('nrevx',ierr)

      call fiReadInt(string,nrevy,ierr)
      call fiDefaultMsg('nrevy',ierr)

      call fiReadInt(string,fnrevy,ierr)
      call fiDefaultMsg('fnrevy',ierr)    

      call fiReadInt(string,iread,ierr)
      call fiDefaultMsg('iread',ierr)

      call fiReadInt(string,iseed,ierr)
      call fiDefaultMsg('iseed',ierr)

      call fiReadInt(string,in,ierr)
      call fiDefaultMsg('in',ierr)

      call fiReadInt(string,inn,ierr)
      call fiDefaultMsg('inn',ierr)
      
      call fiReadInt(string,iprimary_min,ierr)
      call fiDefaultMsg('iprimary_min',ierr)      

      call fiReadInt(string,i_ini_tra,ierr)
      call fiDefaultMsg('i_ini_tra',ierr)      

      call fiReadLog(string,new,ierr)
      call fiDefaultMsg('new',ierr)

      call fiReadLog(string,flowsteady,ierr)
      call fiDefaultMsg('flowsteady',ierr)

      call fiReadLog(string,initial_slt,ierr)
      call fiDefaultMsg('initial_slt',ierr)
      
      call fiReadLog(string,advection,ierr)
      call fiDefaultMsg('advection',ierr)      
      
      if (myrank == 0) then
        write(iunit2,'(/," *LBM")')
        write(iunit2,'(''  ntimes      = '',i6)') ntimes
        write(iunit2,'(''  bsat        = '',1pe12.4)') bsat
        write(iunit2,'(''  nbsat       = '',1pe12.4)') nbsat
!       write(iunit2,'(''  nbsat       = '',i6)') nbsat
        write(iunit2,'(''  rhol        = '',1pe12.4)') rhol
        write(iunit2,'(''  rhor        = '',1pe12.4)') rhor
        write(iunit2,'(''  uin         = '',1pe12.4)') uin
        write(iunit2,'(''  ff          = '',1pe12.4)') ff    
        write(iunit2,'(''  ibndtp      = '',i6)') ibndtp              
        write(iunit2,'(''  tau0        = '',1pe12.4)') tau0
        write(iunit2,'(''  epsss       = '',1pe12.4)') epsss
        write(iunit2,'(''  icheck      = '',i6)') icheck
        write(iunit2,'(''  iwrite      = '',i6)') iwrite
        write(iunit2,'(''  ishift     = '',i6)') ishift
        write(iunit2,'(''  nrevx      = '',i6)') nrevx 
        write(iunit2,'(''  nrevy      = '',i6)') nrevy 
        write(iunit2,'(''  fnrevy     = '',i6)') fnrevy                        
        write(iunit2,'(''  iread       = '',i6)') iread
        write(iunit2,'(''  iseed       = '',i6)') iseed
        write(iunit2,'(''  in          = '',i6)') in
        write(iunit2,'(''  inn         = '',i6)') inn
        write(iunit2,'(''  iprimary_min= '',i6)') iprimary_min        
        write(iunit2,'(''  i_ini_tra   = '',i6)') i_ini_tra
        write(iunit2,'(''  new         = '',l6)') new
        write(iunit2,'(''  flowsteady  = '',l6)') flowsteady
        write(iunit2,'(''  initial_slt = '',l6)') initial_slt
        write(iunit2,'(''  advection   = '',l6)') advection
      endif
!....................

    case default
      if (myrank == 0) &
      write(*,*) 'error reading input file: keyword not found-stop!'
      stop
...
!#include "definitions.h"
  integer, parameter:: MAXSTRINGLENGTH=256
