...
!     using residual properties.  A residual prperty is defined as the
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      SUBROUTINE HENRY(xmole,x1m,tx,pco2,phi,rkh,poyn)

      implicit real*8 (a-h,o-z)

      ams = 1.801534D-02 !h2o
      ama = 4.40098D-02 !co2
...
!-------crovetto form
!       rkH = 2.45621 + (948.4254679-458199.88483*otk)*otk + (1008.599*
!    .  (1 - 0.001545294*tk)**0.333333333)*otk

!-------new fit 1 - fixed Vco2 = 35 cm^3/mol
        rkH = 9.38406 - (6192.46+132377.0*otk)*otk + (5885.39* &
        (1 - 0.001545294*tk)**0.333333333)*otk
      endif
...
!C
   ...
   10 IGOOD=2
      WRITE(6,1) ' ',T
    1 FORMAT(A1,'TEMPERATURE = ',E13.6,'  OUT OF RANGE IN SAT ')
      RETURN
      END subroutine SAT
...
!      Z=Y+ DSQRT(ZP)
    ...
    1 IGOOD=2
      WRITE(6,2)TF
      WRITE(7,2)TF
    2 FORMAT(' TEMPERATURE = ',E13.6,'  OUT OF RANGE IN COWAT')
...
!
!     PRINT 10,T,P,PS,CON
   10 FORMAT(5H T = ,E13.6,5H P = ,E13.6,6H PS = ,E13.6,7H CON = ,E13.6)
!
      RETURN
    1 CONTINUE
!     PRINT 2,T,P,PS
    2 FORMAT(8H AT T = ,E13.6,5H P = ,E13.6,19H IS LESS THAN PS = ,E13.6)
      RETURN
